...
**Gateway URL**: Use the gateway URL from the "Current configuration" section of your system prompt (e.g., `http://127.0.0.1:8888`). Do not hardcode it - it may vary.

### Steps

1. **Extract the employee name** from the user's message (e.g., `/sync @cuttlefish-dev` → `cuttlefish-dev`)
...
User: `/sync @cuttlefish-dev`
You: *[fetches sessions via API, finds cuttlefish-dev's latest session, reads messages]* "Here's what happened in the latest conversation with @cuttlefish-dev: [summary]. Want me to follow up on anything?"
