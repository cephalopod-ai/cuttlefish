...
Read `~/.cuttlefish/logs/gateway.log` and look for:
...
Read `~/.cuttlefish/config.yaml` and verify:
- The file is valid YAML (no syntax errors)
- The `gateway.port` field is a valid number (default `8888`)
...
- Read the session registry at `~/.cuttlefish/sessions/registry.db` (SQLite)
...
- Check if `~/.cuttlefish/tmp/` contains stale files that should be cleaned up
...
```bash
cuttlefish stop && cuttlefish start
```
...
Delete all contents of `~/.cuttlefish/tmp/` but keep the directory itself.
...
For understanding {{portalName}}'s architecture and component relationships, refer to the documentation in `~/.cuttlefish/docs/` if available.
