...
All cron jobs are stored in `~/.cuttlefish/cron/jobs.json` as a JSON array of job objects. If the file does not exist, create it with an empty array `[]`.
...
1. Read the current `~/.cuttlefish/cron/jobs.json` (or initialize as `[]` if missing).
...
10. Write the updated array back to `~/.cuttlefish/cron/jobs.json`.
