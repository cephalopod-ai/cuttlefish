...
{{portalName}}'s engines operate within `~/.cuttlefish/` and can modify any file in that directory. This enables {{portalName}} to update its own configuration, create skills, manage cron jobs, and restructure the organization at runtime.
...
Engines have full file access within `~/.cuttlefish/`. To avoid breaking the gateway:
...
- Modify files outside `~/.cuttlefish/` unless explicitly instructed by the user
...
```
1. Create ~/.cuttlefish/org/<department>/<name>.yaml with required fields
