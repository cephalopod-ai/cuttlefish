...
{{portalName}} supports scheduled AI jobs defined in `~/.cuttlefish/cron/jobs.json`.
...
Each job execution is logged to `~/.cuttlefish/cron/runs/<jobId>.jsonl`. Each line is a JSON object:
...
[
  ...
  },
  {
    ...
    "engine": "claude",
    "employee": "{{portalSlug}}",
    "prompt": "Review all skills in ~/.cuttlefish/skills/ and suggest improvements or removals for unused skills."
  }
]
```
