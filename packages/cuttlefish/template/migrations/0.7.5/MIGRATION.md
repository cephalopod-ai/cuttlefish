...
Update `cuttlefish.version` in `config.yaml` from the previous migration version to `"0.7.5"`.
...
- `POST /api/files/transfer` endpoint for remote file sharing between Cuttlefish instances
...
1. **Config**: Update `cuttlefish.version` to `"0.7.5"`. No other config changes required.
