...
import { effortLevelsForModel, engineAvailable, isKnownEngine, engineUnavailableMessage, getModelRegistry } from "../shared/models.js";
  ...
  try {
    ...
    const attemptEscalation = async (trigger: "stall" | "usage", detail: string): Promise<boolean> => {
      ...
      const fallbackDecision = resolveModelFallback({
        ...
        // Registry-aware, not engine-only: a ladder rung names a specific model
        // id, and this install's registry is what says whether that id is
        // dispatchable. Checking only the engine let escalation hand the CLI a
        // `--model` it never advertised (e.g. a pinned `claude-opus-5` rung on
        // an install whose Claude registry still only carries the `opus` alias),
        // turning an automatic fallback into a hard failure. Skipping the rung
        // lets the ladder move on to one this install can actually run.
        isAvailable: (e, model) => {
          if (!isKnownEngine(e) || !context.sessionManager.getEngine(e) || !engineAvailable(config, e)) return false;
          const models = getModelRegistry(config)[e]?.models;
          // No registry entry for the engine → fall back to the engine-only
          // verdict rather than blocking every rung for it.
          return !models || models.length === 0 || models.some((m) => m.id === model);
        },
      });
