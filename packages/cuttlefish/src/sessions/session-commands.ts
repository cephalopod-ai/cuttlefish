...
export async function handleSessionCommand(
  ...
  if (text === "/doctor" || text.startsWith("/doctor ")) {
    ...
    const info = [
      ...
      ...(deps.config.engines.antigravity ? [`Antigravity: ${deps.config.engines.antigravity.model ?? "gemini-3.8-flash-medium"}`] : []),
      ...(deps.config.engines.grok ? [`Grok: ${describeGrokModelForOperator(deps.config.engines.grok.model)}`] : []),
      ...(deps.config.engines.ollama ? [`Ollama: ${deps.config.engines.ollama.model ?? "gemma4:26b"}`] : []),
