...
function buildStatusLineRecorderCommand(sessionId: string, dir: string): string {
  const script = [
    ...
    `const safe={captured_at:new Date().toISOString(),cuttlefish_session_id:id,model:parsed.model||prev.model,version:parsed.version||prev.version,rate_limits:parsed.rate_limits||prev.rate_limits,context_window:usefulCtx?parsed.context_window:(prev.context_window||parsed.context_window),cost:parsed.cost||prev.cost};`,
...
export function seedTrust(claudeJsonPath: string, projectDir: string): void {
  ...
  // pre-Cuttlefish original (no timestamped proliferation; first write wins).
  const backupPath = `${claudeJsonPath}.cuttlefish-backup`;
