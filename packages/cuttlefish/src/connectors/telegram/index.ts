...
export class TelegramConnector implements Connector {
  ...
  constructor(config: TelegramConnectorConfig) {
    ...
    // operator to explicitly authorize users by Telegram numeric ID.
    this.allowedUsers = new Set(config.allowFrom ?? []);
    if (this.allowedUsers.size === 0) {
      logger.warn("[telegram] No allowFrom configured — ignoring ALL inbound messages. Set connectors.telegram.allowFrom to authorize user IDs.");
    }
