...
export class GrokInteractiveEngine implements InterruptibleEngine, PtyViewEngine {
  ...
  async run(opts: EngineRunOpts): Promise<EngineResult> {
    ...
    if (!cuttlefishSessionId) throw new Error("GrokInteractiveEngine.run requires opts.sessionId");
    ...
    if (warm && this.spawnParamsChanged(cuttlefishSessionId, opts, grokSessionId)) {
    ...
    const onParsed = (parsed: GrokParsedLine) => {
      ...
      if (parsed.sessionId && !grokSessionId) {
        ...
        this.spawnParams.set(cuttlefishSessionId, { model: opts.model, effortLevel: opts.effortLevel, sessionId: grokSessionId });
      }
    ...
    const attachTail = (filePath: string, fromBeginning = false) => {
      ...
      if (grokSessionId) this.spawnParams.set(cuttlefishSessionId, { model: opts.model, effortLevel: opts.effortLevel, sessionId: grokSessionId });
      let offset = 0;
      if (!fromBeginning) {
    ...
    if (existingTranscript) {
      ...
      const discover = setInterval(() => {
        ...
        if (fresh.length === 1) {
          ...
          logger.warn(`GrokInteractiveEngine: ambiguous fresh transcripts for ${cuttlefishSessionId}; waiting for a unique candidate`);
    ...
    if (warm) {
      ...
      const handle = this.spawn(cuttlefishSessionId, opts, grokSessionId, systemPromptOverride);
  ...
  private wireProcToStream(cuttlefishSessionId: string, proc: pty.IPty): PtyHandle {
  ...
  ensureIdleSpawn(cuttlefishSessionId: string, opts: PtyIdleSpawnOpts): void {
    ...
    if (this.lifecycle.getWarm(cuttlefishSessionId) && !this.spawnParamsChanged(cuttlefishSessionId, opts, grokSessionId)) return;
    if (this.lifecycle.getWarm(cuttlefishSessionId)) this.lifecycle.releaseSession(cuttlefishSessionId);
    const handle = this.spawn(cuttlefishSessionId, opts, grokSessionId);
