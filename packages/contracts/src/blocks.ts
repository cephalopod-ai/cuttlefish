...
const OPS = new Set<ChatBlockEnvelope["op"]>(["put", "patch", "remove"]);
...
export function isChatBlock(value: unknown): value is ChatBlock {
  ...
  if (typeof value.type !== "string" || !BLOCK_TYPES.has(value.type as ChatBlockType)) return false;
...
export function validateBlockEnvelope(input: unknown): BlockValidationResult {
  ...
  if (!OPS.has(op as ChatBlockEnvelope["op"])) return { ok: false, error: "block op must be put, patch, or remove" };
...
export const blockFallbackText = blockFallbackContent;
