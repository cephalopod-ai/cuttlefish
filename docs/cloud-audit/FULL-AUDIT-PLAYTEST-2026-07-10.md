...
## Source-location addendum — 2026-09-06

MOD-KANBAN-20260906 extracts `mapBoardTicket`, board loading and save serialization
from `packages/web/src/routes/kanban/page.tsx` to
`packages/web/src/routes/kanban/kanban-board-data.ts`. July line references above
remain historical observations.

H4 is **fixed, locally verified** in MOD-KANBAN-20260906 (feature ledger 0123).
The Chromium fixture replay reproduced the original board-hiding failure before
repair. After repair, valid tickets in both departments remain visible with an
explicit malformed-record warning. A damaged department cannot send a partial
whole-board save; another department can save, and repair plus reload restores
saving. Direct regressions cover malformed active/deleted records, versions,
prototype keys and duplicate IDs. The patch uses a warning and save guard; it
does not invent placeholder tickets. See `kanban-board-data.test.ts` and local
`docs/audits/2026-09-06-kanban-source-modularization.md` for validation and limits.
Other July findings are not closed by this scoped addendum.
