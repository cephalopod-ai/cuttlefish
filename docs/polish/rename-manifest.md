...
|---|---|---|---|---|
| `.claude/`, `.agents/` tracked support surfaces | None proposed | These may appear tool-specific, but they are currently part of the repo's agent/tooling surface. `.fissure/` is local-only tooling and is ignored. | medium | Public staging already treats broad local artifacts separately; changing the tracked support paths could confuse tool integrations. |
