...
**Files:**
- Modify: `packages/cuttlefish/src/shared/types.ts:195-215` (Employee interface)
- Modify: `packages/cuttlefish/src/shared/types.ts` (append new interfaces)
...
Run: `cd ~/Projects/cuttlefish && pnpm typecheck`
...
**Files:**
- Modify: `packages/cuttlefish/src/gateway/org.ts:28-41` (inside scanOrg YAML extraction)
...
```bash
git add packages/cuttlefish/src/gateway/org.ts
...
**Files:**
- Create: `packages/cuttlefish/src/gateway/__tests__/org-hierarchy.test.ts`
...
Run: `cd ~/Projects/cuttlefish && pnpm --filter cuttlefish-cli test -- --run src/gateway/__tests__/org-hierarchy.test.ts`
Expected: FAIL - module `../org-hierarchy.js` does not exist

- [ ] **Step 3: Commit failing tests**

```bash
git add packages/cuttlefish/src/gateway/__tests__/org-hierarchy.test.ts
...
**Files:**
- Create: `packages/cuttlefish/src/gateway/org-hierarchy.ts`
...
```bash
git add packages/cuttlefish/src/gateway/org-hierarchy.ts
...
**Files:**
- Modify: `packages/cuttlefish/src/gateway/api.ts:891-924` (GET /api/org handler)
...
**Files:**
- Modify: `packages/cuttlefish/src/gateway/api.ts:927-948` (GET /api/org/employees/:name handler)
...
**Files:**
- Modify: `packages/cuttlefish/src/sessions/context.ts:44-57` (buildContext signature + org section)
- Modify: `packages/cuttlefish/src/sessions/context.ts:319-360` (buildOrgContext function)
- Modify: `packages/cuttlefish/src/sessions/context.ts:206-238` (buildEmployeeIdentity function)
...
```bash
git add packages/cuttlefish/src/sessions/context.ts
...
**Files:**
- Modify: `packages/cuttlefish/src/sessions/manager.ts:242-252` (buildContext call)
- Modify: `packages/cuttlefish/src/gateway/api.ts:1900-1919` (web streaming buildContext call)
...
```bash
git add packages/cuttlefish/src/sessions/manager.ts packages/cuttlefish/src/gateway/api.ts
...
Run: `cd ~/Projects/cuttlefish && grep -r "useOrg\(\)" packages/web/src/ --include="*.tsx" --include="*.ts"`
...
Run: `cd ~/Projects/cuttlefish && pnpm build`
Expected: PASS

- [ ] **Step 3: Run tests**

Run: `cd ~/Projects/cuttlefish && pnpm test`
...
**Files:**
- Modify: `~/.cuttlefish/skills/management/SKILL.md`
...
```bash
git add ~/.cuttlefish/skills/management/SKILL.md
...
**Files:**
- Create: `~/.cuttlefish/migrations/hierarchical-org/MIGRATION.md` (per spec convention)
...
```bash
git add ~/.cuttlefish/migrations/hierarchical-org/MIGRATION.md
...
**Files:**
- Modify: `~/.cuttlefish/org/**/*.yaml` (existing employee files)
...
Run: `curl -s http://0.0.0.0:8888/api/org | jq '.hierarchy.warnings'`
Expected: Empty array or only informational warnings

- [ ] **Step 3: Commit**

```bash
git add ~/.cuttlefish/org/
...
Run: `curl -s http://0.0.0.0:8888/api/org | jq '{root: .hierarchy.root, employeeCount: (.employees | length), warnings: .hierarchy.warnings}'`
Expected: Shows root, employee count matches org, no critical warnings

- [ ] **Step 4: Test employee detail endpoint**

Run: `curl -s http://0.0.0.0:8888/api/org/employees/content-lead | jq '{name, parentName, directReports, depth, chain}'`
