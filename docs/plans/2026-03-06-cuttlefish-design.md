# Cuttlefish Design Document

**Date:** 2026-03-06
**Status:** Approved
**Package:** `cuttlefish-cli`

---

## 1. Overview

Cuttlefish is a lightweight, open-source AI gateway daemon that orchestrates Claude Code CLI and Codex SDK to create an autonomous, self-organizing AI workforce.

### Core Principles

1. **Zero opinions on AI** - Cuttlefish never touches the agentic loop. No ReAct implementation, no tool execution, no context management, no memory system. Claude Code and Codex handle all intelligence. Cuttlefish is a bus, not a brain.
...
3. **Organic growth** - No predefined structure. Cuttlefish bootstraps an organization based on the user's actual needs. Departments, employees, ranks, boards - all created on demand by Cuttlefish's management skill, not hardcoded.

### Mental Model

You are the CEO. Cuttlefish is your COO. He hires, manages, and orchestrates a team of AI employees - each backed by Claude Code or Codex - to handle your work across projects, platforms, and domains.
...
|-----------|---------|---------|
| Core | `cuttlefish-cli` | CLI + gateway daemon + engines + connectors + cron |
| Web UI | `@cuttlefish/web` | Next.js + Tailwind dashboard, statically exported |
| Template | `template/` | Init files copied to `~/.cuttlefish/` on setup |

### Single Process Runtime

`cuttlefish start` boots one Node.js process that runs:
...
Connector normalizes -> { source, channel, thread, user, text, attachments }
    |
    v
Gateway checks: is this a Cuttlefish command (/new, /status)?
    | YES -> handle internally, respond
    | NO  v
    |
Router: who is this for?
    | @employee-name mentioned? -> look up employee persona
    | No mention? -> use channel/DM session (Cuttlefish or last active persona)
    |
    v
Session Manager
    ...
    | 6. Spawn engine process (one-shot, cwd: ~/.cuttlefish/)
    |
    v
...
Default: `8888`. Configurable in `config.yaml`.
...
|------|---------|-----------|-----------------|
| Main | DM to Cuttlefish on Slack | Persistent forever | Same session_id resumed on every DM |
...
- **`/new` command.** User types `/new` in any context -> Cuttlefish creates a new session ID, stops resuming the old one. Old session remains in engine's native storage but Cuttlefish no longer references it.
...
const thread = resumeId
  ...
  : codex.startThread({ workingDirectory: "~/.cuttlefish/" });
const result = await thread.run(prompt);
```

### Adding New Engines

One new file implementing the `Engine` interface. The gateway is engine-agnostic. Any engine can be Cuttlefish's default brain - configurable in `config.yaml`.
...
- Attachments: downloaded from Slack URL to `~/.cuttlefish/tmp/`
...
Before routing, scan message text for `@employee-name` patterns. Match against `~/.cuttlefish/org/` employee registry (loaded by gateway, refreshed by file watcher). If matched, spawn session with that employee's persona. If not matched, use default (Cuttlefish).
...
### Job Definition (`~/.cuttlefish/cron/jobs.json`)
...
| `employee` | no | Employee persona (null = Cuttlefish) |
...
6. Logs run to `~/.cuttlefish/cron/runs/<jobId>.jsonl`
...
### Directory Structure (Created by Cuttlefish on Demand)
...
persona: |
  You are Sarah, the SEO Specialist at Cuttlefish's organization.
...
|------|-----------------|----------------|
| Executive (Cuttlefish) | Talk to anyone, anywhere | Hire/fire, create departments, promote, full org control |
...
Enforcement is via persona instructions (honor system). The AI follows rank constraints defined in the persona YAML. Cuttlefish reviews and course-corrects if needed.

### Gateway Awareness

- On startup + file watch: scans `~/.cuttlefish/org/`, builds in-memory map of `employee name -> { persona, engine, model, department }`
...
| `skill-creator` | Thin wrapper - defers to Claude Code's native skill creation or Codex's native capabilities. Ensures output lands in `~/.cuttlefish/skills/` and follows conventions |
| `self-heal` | Diagnose and fix Cuttlefish issues. Read logs, check config, fix broken state |
| `onboarding` | First-run setup. Detect OpenClaw, analyze and propose migration, scaffold org |

### Discovery

CLAUDE.md and AGENTS.md reference `~/.cuttlefish/skills/` and list available skills. The engine reads the relevant SKILL.md when it needs a capability. Creating new skills = creating a new directory with SKILL.md (via skill-creator or native engine commands).

---

## 9. Initialization Template

### What `cuttlefish setup` Creates
...
Installing LaunchAgent...
  [check] com.cuttlefish.gateway.plist -> ~/Library/LaunchAgents/

Setup complete! Run "cuttlefish start" to boot the gateway.
Open http://localhost:8888 to get started.
```
...
**Chat (`/chat`)** - Primary interaction surface. Sidebar with conversation list (Cuttlefish + employees). Full chat interface. Supports `@employee` mentions, `/new`, `/status`. Input box with engine/model indicator. Onboarding flow lives here on first run.

**Sessions (`/sessions`)** - List all sessions with status. Click for detail view with chat history, metadata, follow-up input.

**Organization (`/org`)** - Interactive tree: Cuttlefish -> departments -> employees with rank badges. Click employee for detail panel (persona, tasks, sessions, engine config). Click department for board view (kanban/list of tasks).

**Cron (`/cron`)** - Job list with enable/disable toggle, schedule, status, history. Click for run history and config editing.

**Skills (`/skills`)** - Grid of skill cards. Click for SKILL.md content. "Create Skill" button triggers chat with Cuttlefish.
...
| Command | Action |
|---------|--------|
| `cuttlefish setup` | Check deps, install missing, create `~/.cuttlefish/`, install LaunchAgent |
| `cuttlefish start` | Start gateway in foreground (default) |
| `cuttlefish start --daemon` | Start gateway as background daemon (PID file at `~/.cuttlefish/gateway.pid`) |
| `cuttlefish stop` | Stop the gateway daemon |
| `cuttlefish status` | Show gateway status, active sessions, cron summary |

---

## 12. Error Handling

- Engine process exits non-zero: add X reaction, send error message to connector, log to `~/.cuttlefish/logs/`
...
1. `cuttlefish setup` installs everything, opens web UI
2. `/chat` page opens with Cuttlefish's welcome message
3. Cuttlefish (via engine) runs interactive onboarding:
...
6. Cuttlefish presents migration proposal with recommended org structure
7. User approves/tweaks what to migrate (skills, cron, knowledge - each opt-in)
8. Cuttlefish scaffolds the org and migrates selected items
...
Slack file attachments are downloaded to `~/.cuttlefish/tmp/` and included as file paths in the prompt. Claude Code reads files/images natively.

### Hot-Reload

The gateway watches `~/.cuttlefish/` for changes:
...
Logs to both file (`~/.cuttlefish/logs/gateway.log`) and stdout when running in foreground.
