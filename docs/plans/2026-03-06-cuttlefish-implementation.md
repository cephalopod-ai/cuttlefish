# Cuttlefish Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build Cuttlefish - a lightweight AI gateway daemon that orchestrates Claude Code CLI and Codex SDK as an autonomous, self-organizing AI workforce.

**Architecture:** Monorepo with two packages (`cuttlefish-cli` core + `@cuttlefish/web` dashboard). Single Node.js process gateway that spawns engine child processes, routes messages from connectors, runs cron jobs, and serves a static Next.js web UI. All state lives in `~/.cuttlefish/` as YAML, JSON, and SQLite files.
...
**Design Doc:** `docs/plans/2026-03-06-cuttlefish-design.md`
...
### Task 1.2: Create cuttlefish-cli package scaffold

**Files:**
- Create: `packages/cuttlefish/package.json`
- Create: `packages/cuttlefish/tsconfig.json`
- Create: `packages/cuttlefish/bin/cuttlefish.ts`
- Create: `packages/cuttlefish/src/shared/types.ts`
- Create: `packages/cuttlefish/src/shared/paths.ts`
- Create: `packages/cuttlefish/src/shared/config.ts`
- Create: `packages/cuttlefish/src/shared/logger.ts`
...
```json
{
  ...
  "license": "MIT",
  "bin": {
    "cuttlefish": "./dist/bin/cuttlefish.js"
  },
...
Define all core interfaces: `Engine`, `EngineResult`, `EngineRunOpts`, `Connector`, `IncomingMessage`, `Attachment`, `Target`, `Session`, `CronJob`, `CronDelivery`, `Employee`, `Department`, `CuttlefishConfig`.
...
export const CUTTLEFISH_HOME = path.join(os.homedir(), ".cuttlefish");
export const CONFIG_PATH = path.join(CUTTLEFISH_HOME, "config.yaml");
export const SESSIONS_DB = path.join(CUTTLEFISH_HOME, "sessions", "registry.db");
export const CRON_JOBS = path.join(CUTTLEFISH_HOME, "cron", "jobs.json");
export const CRON_RUNS = path.join(CUTTLEFISH_HOME, "cron", "runs");
export const ORG_DIR = path.join(CUTTLEFISH_HOME, "org");
export const SKILLS_DIR = path.join(CUTTLEFISH_HOME, "skills");
export const DOCS_DIR = path.join(CUTTLEFISH_HOME, "docs");
export const LOGS_DIR = path.join(CUTTLEFISH_HOME, "logs");
export const TMP_DIR = path.join(CUTTLEFISH_HOME, "tmp");
export const PID_FILE = path.join(CUTTLEFISH_HOME, "gateway.pid");
export const TEMPLATE_DIR = path.join(__dirname, "..", "..", "template");
```

**Step 5: Create config loader (`src/shared/config.ts`)**

Reads and parses `~/.cuttlefish/config.yaml` using js-yaml. Returns typed `CuttlefishConfig`. Throws helpful error if file doesn't exist (suggests running `cuttlefish setup`).

**Step 6: Create logger (`src/shared/logger.ts`)**

Simple logger that writes to both `~/.cuttlefish/logs/gateway.log` (append) and stdout. Supports levels: debug, info, warn, error. Timestamps each line. Respects config `logging.level` and `logging.stdout` settings.

**Step 7: Create CLI entry point (`bin/cuttlefish.ts`)**
...
program.command("setup").description("Initialize Cuttlefish and install dependencies").action(() => {
...
Run: `cd packages/cuttlefish && pnpm build`
Expected: compiles to `dist/` without errors

Run: `node dist/bin/cuttlefish.js --help`
...
```bash
git add -A
git commit -m "feat: scaffold cuttlefish-cli package with shared types"
```
...
**Files:**
- Create: `packages/cuttlefish/src/sessions/registry.ts`

**Step 1: Write registry module**

Implements:
- `initDb()` - creates SQLite database at `~/.cuttlefish/sessions/registry.db`, runs CREATE TABLE IF NOT EXISTS
- `createSession(opts)` - inserts new session, returns Session
- `getSession(id)` - get by Cuttlefish session ID
...
**Files:**
- Create: `packages/cuttlefish/src/engines/types.ts` (extract from shared/types.ts if needed)
- Create: `packages/cuttlefish/src/engines/claude.ts`
...
**Files:**
- Create: `packages/cuttlefish/src/engines/codex.ts`
...
**Files:**
- Create: `packages/cuttlefish/src/sessions/manager.ts`
- Create: `packages/cuttlefish/src/sessions/context.ts`
- Create: `packages/cuttlefish/src/sessions/queue.ts`
...
export function buildContext(opts: {
  ...
  let ctx = `You are Cuttlefish, a personal AI assistant.\n`;
...
**Files:**
- Create: `packages/cuttlefish/src/gateway/server.ts`
- Create: `packages/cuttlefish/src/gateway/api.ts`
- Create: `packages/cuttlefish/src/gateway/watcher.ts`
- Create: `packages/cuttlefish/src/gateway/lifecycle.ts`
...
- `startDaemon()` - fork child process, write PID to `~/.cuttlefish/gateway.pid`, parent exits
...
**Files:**
- Modify: `packages/cuttlefish/bin/cuttlefish.ts`
- Create: `packages/cuttlefish/src/cli/setup.ts`
- Create: `packages/cuttlefish/src/cli/start.ts`
- Create: `packages/cuttlefish/src/cli/stop.ts`
- Create: `packages/cuttlefish/src/cli/status.ts`
...
5. Create `~/.cuttlefish/` directory structure by copying from `template/`
6. Initialize empty SQLite database with schema
7. Create empty `cron/jobs.json` with `[]`
8. Install LaunchAgent plist to `~/Library/LaunchAgents/com.cuttlefish.gateway.plist`
9. Print summary

**Step 2: Write start command (`cli/start.ts`)**

Check if `~/.cuttlefish/` exists (suggest `cuttlefish setup` if not). If `--daemon` flag, call `lifecycle.startDaemon()`. Otherwise call `lifecycle.startForeground()`.
...
**Step 5: Wire into bin/cuttlefish.ts**

Replace TODO placeholders with actual imports and calls.

**Step 6: Build and test end-to-end**

Run: `pnpm build && node dist/bin/cuttlefish.js setup`
Expected: creates `~/.cuttlefish/` directory with all template files

Run: `node dist/bin/cuttlefish.js start`
Expected: gateway starts on port 8888, logs to stdout

Run (in another terminal): `curl http://localhost:8888/api/status`
Expected: JSON response with gateway status

Run: `node dist/bin/cuttlefish.js status`
...
Create the brain - the files that make engines understand Cuttlefish.

### Task 4.1: Write CLAUDE.md and AGENTS.md

**Files:**
- Create: `packages/cuttlefish/template/CLAUDE.md`
- Create: `packages/cuttlefish/template/AGENTS.md`

**Step 1: Write CLAUDE.md**

This is the most important file in the project. It tells every Claude Code session who they are and how Cuttlefish works. Contents:

- Identity: "You are Cuttlefish, a personal AI assistant and COO of an AI organization"
- The `~/.cuttlefish/` folder structure and what each part does
- How to use skills (read SKILL.md from `~/.cuttlefish/skills/`)
...
**Files:**
- Create: `packages/cuttlefish/template/skills/management/SKILL.md`
- Create: `packages/cuttlefish/template/skills/cron-manager/SKILL.md`
- Create: `packages/cuttlefish/template/skills/skill-creator/SKILL.md`
- Create: `packages/cuttlefish/template/skills/self-heal/SKILL.md`
- Create: `packages/cuttlefish/template/skills/onboarding/SKILL.md`
...
Thin wrapper: defer to Claude Code's native `/skill` command or Codex native capabilities. Ensure output lands in `~/.cuttlefish/skills/<name>/SKILL.md`. Include conventions for writing good SKILL.md files.
...
**Files:**
- Create: `packages/cuttlefish/template/docs/overview.md`
- Create: `packages/cuttlefish/template/docs/architecture.md`
- Create: `packages/cuttlefish/template/docs/skills.md`
- Create: `packages/cuttlefish/template/docs/cron.md`
- Create: `packages/cuttlefish/template/docs/connectors.md`
- Create: `packages/cuttlefish/template/docs/org.md`
- Create: `packages/cuttlefish/template/docs/self-modification.md`

**Step 1: Write each doc**

These are for Cuttlefish's self-awareness. Each doc should be concise and technical:
- `overview.md` - what Cuttlefish is, core principles, how it differs from OpenClaw
...
- `self-modification.md` - what Cuttlefish can edit, how file watchers react, safety considerations

**Step 2: Write default config.yaml template**

Create: `packages/cuttlefish/template/config.default.yaml` with the config from the design doc.
...
**Files:**
- Create: `packages/cuttlefish/src/connectors/slack/index.ts`
- Create: `packages/cuttlefish/src/connectors/slack/threads.ts`
- Create: `packages/cuttlefish/src/connectors/slack/format.ts`
...
- `downloadAttachment(url, token, destDir)` - download Slack file to `~/.cuttlefish/tmp/`, return local path
...
Configure Slack tokens in `~/.cuttlefish/config.yaml`. Start gateway. Send a DM to the bot. Verify: eyes reaction appears, Claude Code runs, result posted, checkmark reaction added.
...
**Files:**
- Create: `packages/cuttlefish/src/cron/jobs.ts`
- Create: `packages/cuttlefish/src/cron/scheduler.ts`
- Create: `packages/cuttlefish/src/cron/runner.ts`

**Step 1: Write job loader (`jobs.ts`)**

Reads and writes `~/.cuttlefish/cron/jobs.json`. Functions:
...
**Files:**
- Create: `packages/cuttlefish/src/gateway/org.ts`

**Step 1: Write org scanner**

Functions:
- `scanOrg()` - recursively scan `~/.cuttlefish/org/`, parse all `.yaml` files, return `Map<string, Employee>`
...
List of conversations. "New Chat" button. Cuttlefish always at top. Conversations from `/api/sessions?source=web`.
...
Interactive collapsible tree from `/api/org`. Cuttlefish at top, departments as nodes, employees as leaves with rank badges.
...
Grid of skill cards from `/api/skills`. Click to view SKILL.md content (rendered markdown). "Create Skill" button opens chat with Cuttlefish.
...
**Files:**
- Modify: `packages/cuttlefish/package.json` (add build script that copies web output)
- Modify: `packages/cuttlefish/src/gateway/server.ts` (serve static files from bundled web)
- Modify: `turbo.json` (ensure web builds before cuttlefish-cli)

**Step 1: Add build pipeline**

In turbo.json, ensure `@cuttlefish/web` builds first (static export), then `cuttlefish-cli` copies the `out/` directory into its own dist.
...
Run: `cuttlefish setup` (fresh `~/.cuttlefish/`)
...
Via chat, tell Cuttlefish "Hire an SEO specialist for marketing." Verify: org files created, employee appears in web UI org page.
...
**After Phase 5** you can talk to Cuttlefish on Slack.
