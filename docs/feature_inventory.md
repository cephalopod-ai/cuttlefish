...
- HR / Org Steward is human-only: only a direct, top-level human-operator session may use `hr-manager`. Agent-session requests and parented child sessions return `403 { code: "hr_human_only" }`; HR is excluded from cross-service discovery/routing and automated board dispatch; leader-ack timeouts fall back to an executive or manual human review instead of paging HR. Automatic executive triage starts on the first configured and available cheap-tier model from the shared model ladder (at low effort when supported), while ordinary COO chats retain the executive's configured model.
...
- A session-scoped parent can poll the normalized API transcript view of a direct
