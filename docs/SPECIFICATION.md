...
| REQ-ORG-APPROVAL-001 | A chat may submit an authenticated org-change proposal, but the resulting approval must be bound to that originating chat and may be approved, rejected, or applied only by an authenticated operator. Conversational text is not an approval decision. | verified | `auth-gate.test.ts`, `scoped-token-forbidden.test.ts`, `hr-steward.test.ts` |
