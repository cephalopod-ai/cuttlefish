...
| RDC-R01 | open | medium | data-integrity | Add FOREIGN KEY (approvals→sessions) + PRAGMA foreign_keys; schema migration deferred (RDC-003 resolved the orphan defect at app level). | `sessions/registry/schema.ts` | 2026-06-30 |
...
| RDC-R04 | open | medium | reliability | Follow-up reliability batch (medium-confidence, not patched): connector-notification swallow masking completion; rate-limit fallback `engineOverride` orphan; recovery requeue `retryCount` reset; concurrent session-create dedupe. | `sessions/manager.ts`, `sessions/rate-limit-handler.ts`, `orchestration/recovery-requeue.ts`, `sessions/registry/sessions.ts` | 2026-06-30 |
