...
### 🐛 Fixes
- **Connector-backed replies no longer hang after Homebrew/npm installs.** The npm publish manifest now includes `assets/hook-relay.mjs`, so gateway boot can restore `~/.cuttlefish/hook-relay.mjs` and Claude Stop hooks can report completed Slack/Discord/Telegram turns.
...
- **Talk — in-app mic setup.** Tapping the mic on a fresh install now opens the shared Whisper-model download flow (reused from chat) instead of dead-ending; `cuttlefish setup` checks for `whisper-cli` + `ffmpeg` and prints install guidance.
...
### 🐛 Fixes
- **Duplicate works for `.cuttlefish` sessions** — the fork slug now handles `.` in the working-directory path.
...
> Big release: the voice-first **Talk** interface, the **Antigravity** engine (replacing Gemini CLI), a dynamic model registry with mid-chat switching, a race-free `cuttlefish restart`, the **Ledger** web theme, and WebSocket reliability work.
...
- **`cuttlefish restart` - detached, in-session-safe restart.** A new command that performs stop -> wait-for-port-free -> start from a reparented helper process, so it survives the gateway killing its own session PTYs (the reason a naive `stop && start` from inside a session used to leave the gateway down). `cuttlefish start` now also auto-restarts cleanly when a gateway is already running, instead of racing the old one's shutdown into `EADDRINUSE`.
...
- **Fast mode is explicitly NOT supported** (any engine). The real `claude` CLI has no `--fast` flag - `/fast` is an interactive, stateful TUI toggle (resets on model switch, org-disablable) with no headless/flag equivalent, and Cuttlefish runs turns headless. Rather than carry dead infra, the concept was dropped end-to-end (no `supportsFast` in the registry, no UI control).

### 🪄 Docs
- README overhaul: Antigravity replaces Gemini throughout, the web stack is corrected to Vite (`:5888`), new features are documented (file attachments, in-app file viewer, per-session engine/model/effort, live context meter, `cuttlefish restart`), and the obsolete `engines.claude.mode` headless opt-out was removed. Em dashes purged across all Markdown (except version-pinned migration records).
...
- **File-read endpoint** `GET /api/files/read?path=` - reads any file on disk (single-user/Tailscale threat model, no allowlist) with a 5 MB size cap and binary detection. Relative paths resolve `~/.cuttlefish` first, then `~/Projects`, then gateway cwd, then literal; absolute/`~` paths used verbatim.
...
### ✨ Features
- **File & image attachments in the web chat, both directions.** Attach any file type (images, PDFs, zip, docs, anything) from the composer via drag-drop, click, or paste - the bytes are stored locally under `~/.cuttlefish/uploads/YYYY-MM-DD/<sessionId>/` and **only the file path** is injected into the engine prompt, so Claude/Codex/Gemini read it with their own tools instead of being fed raw bytes. First-message uploads are re-homed into the session bucket once it's created (no orphans).
...
### 🐛 Fixes
- **`cuttlefish --version` reported the wrong version** - the CLI reads its version from `dist/package.json` (emitted by tsc from the `import pkg from "../package.json"` in `bin/cuttlefish.ts`). v0.15.0 was published with a stale `dist/` built before the version bump, so it reported `0.14.0` and the Homebrew formula's `--version` test would mismatch. Rebuilt after the bump; `dist/package.json` and `cuttlefish --version` now match the package version.

## [0.15.0] - 2026-05-22

### ♻️ Refactor
- **Kill the legacy naming soup** - the repo, home dir, npm package, and binary all converged on **cuttlefish**, but the project folder (`~/Projects/<legacy-name>`) and the inner package (`packages/<legacy-name>`) were the lone holdouts. Renamed the project root to `~/Projects/cuttlefish`, `packages/<legacy-name>` → `packages/cuttlefish`, and the CLI entry `bin/<legacy-name>.ts` → `bin/cuttlefish.ts`. The published npm package (`cuttlefish-cli`), the `cuttlefish` binary, and the `~/.cuttlefish` workspace are unchanged - no action needed for existing installs. All workspace/build/Formula/workflow/release-skill path references updated to match.
...
### 🔧 Improvements
- Codex engine now runs with `--dangerously-bypass-approvals-and-sandbox` - prevents Cuttlefish-managed Codex sessions from being constrained by CLI sandbox/approval defaults
...
### 🐛 Bug Fixes
- Remove `@cuttlefish/web` workspace dependency from published package - was causing `unsupported URL type "workspace:"` error on `npm i -g cuttlefish-cli` (web UI is embedded as static files during build, not a runtime dependency)
...
### ✨ CLI
- `cuttlefish setup` - bootstrap ~/.cuttlefish/ from templates
- `cuttlefish start` / `stop` / `status` - daemon management
- `cuttlefish create` / `list` / `remove` - instance management
- `cuttlefish nuke` - permanent instance deletion with safety prompts
- `cuttlefish migrate` - AI-assisted template migrations
- `cuttlefish skills` - skill discovery + skills.sh integration
